...
int posix_wrapper::execvp(const std::string &file,
  ...
  // the string vector that we send to execvp must be terminated by a null
  // pointer!
  ...
  for (int i = 0; i < vec_size - 1;
       ++i) // the last NULL pointer doesn't have to be free'd
...
std::string posix_wrapper::getenv(const std::string &name) {
  if (char *buf = ::getenv(name.c_str()); buf != nullptr) {
    ...
    throw std::runtime_error("Error retreiving an environment variable");
}
